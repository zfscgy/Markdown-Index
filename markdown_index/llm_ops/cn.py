...
cn_templates = Templates(
    ...
    doc_summary_prompt_template = lambda index, n_words: dedent(f"""\
        你需要根据文档的目录列表（包含了各个章节的标题和内容摘要）生成一段描述，概括文档的主要内容。
        注意：你的描述要尽可能精简，减少不必要的、重复的语句，只提取其中的关键内容，尽可能在{n_words}个字以内。
        【输出要求】：
        请按照如下的JSON格式返回：
        {{
    ...
    text_keywords_prompt_template = lambda query, doc_info: dedent(f"""\
        你需要根据用户的查询请求和文档的摘要（如果有的话）提取关键词，关键词应该准确归纳用户的查询意图，并且可以用于后续的检索中。
        ...
        ------------------
        ```text
        """) + (doc_info if doc_info else "无文档摘要") + dedent("""
        ```
        """),
    ...
    retrieve_block_prompt_template = lambda query, block_list: dedent(f"""\
        ...
        注意，你抽取的内容要尽量完整，不要遗漏和用户请求相关的重要信息。请把重要的语句放在前面。
